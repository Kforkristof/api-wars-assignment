...
def register_user(cursor, username, plain_text_password):
    ...
    cursor.execute('''INSERT INTO users (user_name, pw_hash) VALUES (%(user_name)s, %(hashed_password)s)''',
...
def get_good_hash_by_user_name(cursor, user_name):
    cursor.execute('''SELECT pw_hash FROM users WHERE user_name = %(user_name)s''', {'user_name': user_name})
